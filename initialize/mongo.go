...
func (m *mongo) Initialization() error {
	...
	if global.DYCLOUD_CONFIG.Mongo.IsZap {
		opts = internal.Mongo.GetClientOptions()
	}
	ctx := context.Background()
	client, err := qmgo.Open(ctx, &qmgo.Config{
		Uri:              global.DYCLOUD_CONFIG.Mongo.Uri(),
		Coll:             global.DYCLOUD_CONFIG.Mongo.Coll,
		Database:         global.DYCLOUD_CONFIG.Mongo.Database,
		MinPoolSize:      &global.DYCLOUD_CONFIG.Mongo.MinPoolSize,
		MaxPoolSize:      &global.DYCLOUD_CONFIG.Mongo.MaxPoolSize,
		SocketTimeoutMS:  &global.DYCLOUD_CONFIG.Mongo.SocketTimeoutMs,
		ConnectTimeoutMS: &global.DYCLOUD_CONFIG.Mongo.ConnectTimeoutMs,
		Auth: &qmgo.Credential{
			Username:   global.DYCLOUD_CONFIG.Mongo.Username,
			Password:   global.DYCLOUD_CONFIG.Mongo.Password,
			AuthSource: global.DYCLOUD_CONFIG.Mongo.AuthSource,
		},
	}, opts...)
	if err != nil {
		return errors.Wrap(err, "链接mongodb数据库失败!")
	}
	global.DYCLOUD_MONGO = client
	err = m.Indexes(ctx)
	if err != nil {
		return err
	}
...
func (m *mongo) CreateIndexes(ctx context.Context, name string, indexes [][]string) error {
	collection, err := global.DYCLOUD_MONGO.Database.Collection(name).CloneCollection()
	if err != nil {
	...
	for k1, v1 := range indexMap1 {
		...
		if len(fmt.Sprintf("%s.%s.$%s", collection.Name(), name, v1)) > 127 {
			err = global.DYCLOUD_MONGO.Database.Collection(name).CreateOneIndex(ctx, options.IndexModel{
				Key:          v1,
