...
import (
	...
	request "DYCLOUD/model/system/request"
	"DYCLOUD/utils"
...
func (s *autoCodeHistory) Create(ctx context.Context, info request.SysAutoHistoryCreate) error {
	create := info.Create()
	err := global.DYCLOUD_DB.WithContext(ctx).Create(&create).Error
	if err != nil {
...
func (s *autoCodeHistory) First(ctx context.Context, info common.GetById) (string, error) {
	var meta string
	err := global.DYCLOUD_DB.WithContext(ctx).Model(model.SysAutoCodeHistory{}).Where("id = ?", info.ID).Pluck("request", &meta).Error
	if err != nil {
...
func (s *autoCodeHistory) Repeat(businessDB, structName, Package string) bool {
	var count int64
	global.DYCLOUD_DB.Model(&model.SysAutoCodeHistory{}).Where("business_db = ? and struct_name = ? and package = ? and flag = 0", businessDB, structName, Package).Count(&count)
	return count > 0
}
...
func (s *autoCodeHistory) RollBack(ctx context.Context, info request.SysAutoHistoryRollBack) error {
	...
	err := global.DYCLOUD_DB.Where("id = ?", info.ID).First(&history).Error
	if err != nil {
		return err
	}
	if history.ExportTemplateID != 0 {
		err = global.DYCLOUD_DB.Delete(&model.SysExportTemplate{}, "id = ?", history.ExportTemplateID).Error
		if err != nil {
			return err
		}
	}
	if info.DeleteApi {
		...
		if err != nil {
			global.DYCLOUD_LOG.Error("ClearTag DeleteApiByIds:", zap.Error(err))
		}
	} // 清除API表
	if info.DeleteMenu {
	...
	for key, template := range history.Templates {
		{
			server := filepath.Join(global.DYCLOUD_CONFIG.AutoCode.Root, global.DYCLOUD_CONFIG.AutoCode.Server)
			keys := strings.Split(key, "/")
			key = filepath.Join(keys...)
			key = strings.TrimPrefix(key, server)
		} // key
		{
			web := filepath.Join(global.DYCLOUD_CONFIG.AutoCode.Root, global.DYCLOUD_CONFIG.AutoCode.WebRoot())
			server := filepath.Join(global.DYCLOUD_CONFIG.AutoCode.Root, global.DYCLOUD_CONFIG.AutoCode.Server)
	...
	removeBasePath := filepath.Join(global.DYCLOUD_CONFIG.AutoCode.Root, "rm_file", strconv.FormatInt(int64(time.Now().Nanosecond()), 10))
	for _, value := range history.Templates {
		if !filepath.IsAbs(value) {
			continue
		}
		removePath := filepath.Join(removeBasePath, strings.TrimPrefix(value, global.DYCLOUD_CONFIG.AutoCode.Root))
		err = utils.FileMove(value, removePath)
		if err != nil {
			return errors.Wrapf(err, "[src:%s][dst:%s]文件移动失败!", value, removePath)
		}
	} // 移动文件
	err = global.DYCLOUD_DB.WithContext(ctx).Model(&model.SysAutoCodeHistory{}).Where("id = ?", info.ID).Update("flag", 1).Error
	if err != nil {
...
func (s *autoCodeHistory) Delete(ctx context.Context, info common.GetById) error {
	err := global.DYCLOUD_DB.WithContext(ctx).Where("id = ?", info.Uint()).Delete(&model.SysAutoCodeHistory{}).Error
	if err != nil {
...
func (s *autoCodeHistory) GetList(ctx context.Context, info common.PageInfo) (list []model.SysAutoCodeHistory, total int64, err error) {
	...
	db := global.DYCLOUD_DB.WithContext(ctx).Model(&model.SysAutoCodeHistory{})
...
func (s *autoCodeHistory) DropTable(BusinessDb, tableName string) error {
	if BusinessDb != "" {
		...
		return global.DYCLOUD_DB.Exec("DROP TABLE " + tableName).Error
	}
}
