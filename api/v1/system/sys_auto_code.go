...
func (autoApi *AutoCodeApi) GetDB(c *gin.Context) {
	...
	for _, db := range global.DYCLOUD_CONFIG.DBList {
...
func (autoApi *AutoCodeApi) GetTables(c *gin.Context) {
	...
	businessDB := c.Query("businessDB")
	if dbName == "" {
		dbName = *global.DYCLOUD_ACTIVE_DBNAME
		if businessDB != "" {
			for _, db := range global.DYCLOUD_CONFIG.DBList {
	...
	if err != nil {
		global.DYCLOUD_LOG.Error("查询table失败!", zap.Error(err))
...
func (autoApi *AutoCodeApi) LLMAuto(c *gin.Context) {
	...
	params["mode"] = mode
	path := strings.ReplaceAll(global.DYCLOUD_CONFIG.AutoCode.AiPath, "{FUNC}", "api/chat/ai")
	...
	if err != nil {
		global.DYCLOUD_LOG.Error("大模型生成失败!", zap.Error(err))
	...
	if resStruct.Code == 7 {
		global.DYCLOUD_LOG.Error("大模型生成失败!"+resStruct.Msg, zap.Error(err))
