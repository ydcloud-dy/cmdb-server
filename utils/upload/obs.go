...
func NewHuaWeiObsClient() (client *obs.ObsClient, err error) {
	return obs.New(global.DYCLOUD_CONFIG.HuaWeiObs.AccessKey, global.DYCLOUD_CONFIG.HuaWeiObs.SecretKey, global.DYCLOUD_CONFIG.HuaWeiObs.Endpoint)
}
...
func (o *Obs) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	input := &obs.PutObjectInput{
		PutObjectBasicInput: obs.PutObjectBasicInput{
			ObjectOperationInput: obs.ObjectOperationInput{
				Bucket: global.DYCLOUD_CONFIG.HuaWeiObs.Bucket,
				Key:    filename,
			},
	...
	filepath := global.DYCLOUD_CONFIG.HuaWeiObs.Path + "/" + filename
