...
func (s *autoCodePlugin) Install(file *multipart.FileHeader) (web, server int, err error) {
	...
	if len(serverPlugin) != 0 {
		err = installation(serverPlugin, global.DYCLOUD_CONFIG.AutoCode.Server, global.DYCLOUD_CONFIG.AutoCode.Server)
		if err != nil {
	...
	if len(webPlugin) != 0 {
		err = installation(webPlugin, global.DYCLOUD_CONFIG.AutoCode.Server, global.DYCLOUD_CONFIG.AutoCode.Web)
		if err != nil {
			return webIndex, serverIndex, err
		}
	}
...
func installation(path string, formPath string, toPath string) error {
	...
	var form = filepath.Join(global.DYCLOUD_CONFIG.AutoCode.Root, formPath, path)
	var to = filepath.Join(global.DYCLOUD_CONFIG.AutoCode.Root, toPath, "plugin")
...
func (s *autoCodePlugin) PubPlug(plugName string) (zipPath string, err error) {
	...
	webPath := filepath.Join(global.DYCLOUD_CONFIG.AutoCode.Root, global.DYCLOUD_CONFIG.AutoCode.Web, "plugin", plugName)
	serverPath := filepath.Join(global.DYCLOUD_CONFIG.AutoCode.Root, global.DYCLOUD_CONFIG.AutoCode.Server, "plugin", plugName)
	// 创建一个新的zip文件
	...
	return filepath.Join(global.DYCLOUD_CONFIG.AutoCode.Root, global.DYCLOUD_CONFIG.AutoCode.Server, fileName), nil
}

func (s *autoCodePlugin) InitMenu(menuInfo request.InitMenu) (err error) {
	menuPath := filepath.Join(global.DYCLOUD_CONFIG.AutoCode.Root, global.DYCLOUD_CONFIG.AutoCode.Server, "plugin", menuInfo.PlugName, "initialize", "menu.go")
	...
	err = global.DYCLOUD_DB.Find(&menus, "id in (?)", menuInfo.Menus).Error
	if err != nil {
		return err
	}
...
func (s *autoCodePlugin) InitAPI(apiInfo request.InitApi) (err error) {
	apiPath := filepath.Join(global.DYCLOUD_CONFIG.AutoCode.Root, global.DYCLOUD_CONFIG.AutoCode.Server, "plugin", apiInfo.PlugName, "initialize", "api.go")
	...
	err = global.DYCLOUD_DB.Find(&apis, "id in (?)", apiInfo.APIs).Error
	if err != nil {
		return err
	}
