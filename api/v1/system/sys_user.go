...
func (b *BaseApi) Login(c *gin.Context) {
	...
	// 判断验证码是否开启
	openCaptcha := global.DYCLOUD_CONFIG.Captcha.OpenCaptcha               // 是否开启防爆次数
	openCaptchaTimeOut := global.DYCLOUD_CONFIG.Captcha.OpenCaptchaTimeOut // 缓存超时时间
	...
	if !oc || (l.CaptchaId != "" && l.Captcha != "" && store.Verify(l.CaptchaId, l.Captcha, true)) {
		...
		if err != nil {
			global.DYCLOUD_LOG.Error("登陆失败! 用户名不存在或者密码错误!", zap.Error(err))
			// 验证码次数+1
		...
		if user.Enable != 1 {
			global.DYCLOUD_LOG.Error("登陆失败! 用户被禁止登录!")
			// 验证码次数+1
			global.BlackCache.Increment(key, 1)
...
func (b *BaseApi) TokenNext(c *gin.Context, user system.SysUser) {
	...
	if err != nil {
		global.DYCLOUD_LOG.Error("获取token失败!", zap.Error(err))
		response.FailWithMessage("获取token失败", c)
		return
	}
	if !global.DYCLOUD_CONFIG.System.UseMultipoint {
	...
	if jwtStr, err := jwtService.GetRedisJWT(user.Username); err == redis.Nil {
		if err := jwtService.SetRedisJWT(token, user.Username); err != nil {
			global.DYCLOUD_LOG.Error("设置登录状态失败!", zap.Error(err))
...
func (b *BaseApi) Register(c *gin.Context) {
	...
	if err != nil {
		global.DYCLOUD_LOG.Error("注册失败!", zap.Error(err))
...
func (b *BaseApi) ChangePassword(c *gin.Context) {
	...
	u := &system.SysUser{DYCLOUD_MODEL: global.DYCLOUD_MODEL{ID: uid}, Password: req.Password}
...
func (b *BaseApi) SetUserAuthority(c *gin.Context) {
	...
	j := &utils.JWT{SigningKey: []byte(global.DYCLOUD_CONFIG.JWT.SigningKey)} // 唯一签名
...
func (b *BaseApi) SetUserInfo(c *gin.Context) {
	...
	err = userService.SetUserInfo(system.SysUser{
		DYCLOUD_MODEL: global.DYCLOUD_MODEL{
			ID: user.ID,
		},
...
func (b *BaseApi) ResetPassword(c *gin.Context) {
	...
	if err != nil {
		global.DYCLOUD_LOG.Error("重置失败!", zap.Error(err))
